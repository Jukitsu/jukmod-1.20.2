...
public class ModConfig {
    ...
    private final EntitiesCategory entities;
    private final GameplayCategory gameplay;



    public ModConfig(Jukmod mod, Path file) {
        this.mod = mod;
        ...
        entities = this.registerCategory(new EntitiesCategory(this));
        gameplay = this.registerCategory(new GameplayCategory(this));


        try {
            this.loadConfig();
        } catch (IOException e) {
            logger.error("Failed to load Jukmod config file", e);
        }
    }

    public Jukmod getMod() {
        return mod;
    }
    ...
    public InputStream getConfigResource() {
        InputStream resourceStream = getClass().getClassLoader().getResourceAsStream("assets/" + Jukmod.MOD_ID + "/config.toml");
    ...
    public AnimationsCategory animations() { return animations; }

    public EntitiesCategory entities() { return entities; }

    public GameplayCategory gameplay() { return gameplay; }
}
